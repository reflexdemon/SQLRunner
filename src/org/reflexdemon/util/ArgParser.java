...
public class ArgParser
{
   ...
   /**
	 * Returns values corresponding to the requested key(switch) with the pattern:
	 * 	-switch value1 value2(strict adherence otherwise exception thrown, value1 and value2 are integer values)  
	 */
  public Integer[] getArgValues(String key) {
	   
	   Integer[] switchValues = null; 
	   try {
		   for (int i=0; i<args.length; i++) {
		       ...
		       if (arg.equalsIgnoreCase(key) &&
		             args.length >= i+2) {
		    	   switchValues = new Integer[2];
		    	   switchValues[i] =  Integer.parseInt(args[i+1].trim());		           
		    	   switchValues[i+1] = Integer.parseInt(args[i+2].trim());	
		   ...
		   switchValues = null;
		   System.out.println("Please use \"-switch value1 value2 \" pattern.");
		   
	   } catch(NumberFormatException ex) {
		   
		   switchValues = null;  
		   System.out.println ("Please supply integer values for value1 and value2 in the same order.");
	   }
	   return switchValues;
  }
}
